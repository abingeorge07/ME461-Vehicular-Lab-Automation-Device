...
#define DT_SERVO 60  // in milliseconds
...
void PID_tilt(void){
    ...
    printf("%.2f, %.2f, %.2f, %.2f, %.2f\n", angles[0], change, error, serror_dt, derror_dt);
    // if(error > 1 || error < -1){
        if(((change*GEAR_RATIO)) > SERVO_SAFETY_MAX){
            servoAngle = SERVO_SAFETY_MAX;
        }
        else if(((change*GEAR_RATIO)) < SERVO_SAFETY_MIN){
